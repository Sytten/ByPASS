\section{Structure du code du projet}
	...
	\begin{description}
		\item[Terminal de paiement] Contient des modules pour les entrées claviers (Input), pour la communication avec le serveur par Xbee (Server) et pour gérer les LED (\code{LED})
		\item[Terminal de recharge] Contient le module pour la communication avec le serveur par Xbee (\code{Server})
		\item[Pont] Contient le module pour la communication Xbee avec les terminaux (\code{Xbee}) et le module pour la communication Ethernet avec le serveur (\code{Server})
	\end{description}
	
	Dans tous les cas, les projets contiennent un fichier de configuration (\code{config}) qui permet de configurer la composante facilement. Ils contiennent également un fichier debug pour activer le mode debug (\code{debug} ou \code{BypassDebug}).
