...
#define RFID_MaxPICCs (sizeof(_TypeNamePICC)/sizeof(_TypeNamePICC[0]))
#define RFID_MaxError (sizeof(_ErrorMessage)/sizeof(_ErrorMessage[0]))
...
void RFID::PCD_ReadRegister(uint8_t reg, uint8_t count, uint8_t *values, uint8_t rxAlign)
{
    ...
    (void) m_SPI.write(address);   // Tell RFID which address we want to read
 ...
 */
void RFID::PCD_SetRegisterBits(uint8_t reg, uint8_t mask)
{
 ...
 */
void RFID::PCD_ClrRegisterBits(uint8_t reg, uint8_t mask)
{
    uint8_t tmp = PCD_ReadRegister(reg);
 ...
 */
uint8_t RFID::PCD_CalculateCRC(uint8_t *data, uint8_t length, uint8_t *result)
{
    ...
    while (1) {
        ...
        if (--i == 0) {
            ...
            // Communication with the RFID might be down.
 ...
 */
uint8_t RFID::PCD_TransceiveData(uint8_t *sendData,
                                 uint8_t sendLen,
                                 uint8_t *backData,
                                 uint8_t *backLen,
                                 uint8_t *validBits,
 ...
 * CRC validation can only be done if backData and backLen are specified.
 */
uint8_t RFID::PCD_CommunicateWithPICC(uint8_t command,
                                      uint8_t waitIRq,
                                      uint8_t *sendData,
                                      uint8_t sendLen,
                                      uint8_t *backData,
                                      ...
                                      bool    checkCRC)
{
    ...
    while (1) {
        ...
        if (--i == 0) {
            // The emergency break. If all other condions fail we will eventually terminate on this one after 35.7ms. Communication with the RFID might be down.
            return STATUS_TIMEOUT;
        }
    }
 ...
 */
uint8_t RFID::PICC_RequestA(uint8_t *bufferATQA, uint8_t *bufferSize)
{
 ...
 * Beware: When two PICCs are in the field at the same time I often get STATUS_TIMEOUT - probably due do bad antenna design.
 */
uint8_t RFID::PICC_WakeupA(uint8_t *bufferATQA, uint8_t *bufferSize)
{
 ...
 * Beware: When two PICCs are in the field at the same time I often get STATUS_TIMEOUT - probably due do bad antenna design.
 */
uint8_t RFID::PICC_REQA_or_WUPA(uint8_t command, uint8_t *bufferATQA, uint8_t *bufferSize)
{
    uint8_t validBits;
    uint8_t status;
 ...
 */
uint8_t RFID::PICC_Select(Uid *uid, uint8_t validBits)
{
    bool uidComplete;
    bool selectDone;
    bool useCascadeTag;
 ...
 */
uint8_t RFID::PCD_Authenticate(uint8_t command, uint8_t blockAddr, MIFARE_Key *key, Uid *uid)
{
 ...
 */
uint8_t RFID::MIFARE_Read(uint8_t blockAddr, uint8_t *buffer, uint8_t *bufferSize)
{
 ...
 */
uint8_t RFID::MIFARE_Write(uint8_t blockAddr, uint8_t *buffer, uint8_t bufferSize)
{
    uint8_t result;
 ...
 */
uint8_t RFID::MIFARE_UltralightWrite(uint8_t page, uint8_t *buffer, uint8_t bufferSize)
{
    uint8_t result;
 ...
 */
uint8_t RFID::MIFARE_Decrement(uint8_t blockAddr, uint32_t delta)
{
 ...
 */
uint8_t RFID::MIFARE_Increment(uint8_t blockAddr, uint32_t delta)
{
 ...
 */
uint8_t RFID::MIFARE_Restore(uint8_t blockAddr)
{
 ...
 */
uint8_t RFID::MIFARE_TwoStepHelper(uint8_t command, uint8_t blockAddr, uint32_t data)
{
    uint8_t result;
 ...
 */
uint8_t RFID::MIFARE_Transfer(uint8_t blockAddr)
{
    uint8_t cmdBuffer[2]; // We only need room for 2 bytes.
 ...
 */
uint8_t RFID::PCD_MIFARE_Transceive(uint8_t *sendData, uint8_t sendLen, bool acceptTimeout)
{
    uint8_t result;
 ...
 */
uint8_t RFID::PICC_GetType(uint8_t sak)
{
